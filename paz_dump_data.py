...
def usage():
    print('\nUsage: python paz_dump_data.py paz_COSAR_product outputfile [l0 lN p0 pN] -res RESFILE')
    print( '  where paz_COSAR_product is the input filename')
...
def read_res_data(resFile):
    res = ResData(resFile)
    #print(res.processes)
    l = res.processes['readfiles']['Number_of_lines_original']
    p = res.processes['readfiles']['Number_of_pixels_original']
    return l,p
...
IMAGE_LINES, LINE_SAMPLES = read_res_data(resFile)
